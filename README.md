...
CI環境を作成します。
Jenkinsには、MultiブランチPipelineのジョブを作成するので、すでに、リポジトリが作成されている必要があります。
そのため、上記の開発キットの作成を先に実行して、リポジトリがある状態で、実行してください。

```
# CI環境を作成
./bin/setup-ci.sh
```
このスクリプトでは、ansibleでCI環境を作成します。ansibleを実行するために、VagrantでVM内で、ansibleを実行します。
